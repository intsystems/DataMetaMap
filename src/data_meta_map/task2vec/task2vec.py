...
import logging
from data_meta_map.task2vec import variational
...
def task2vec(probe_network, dataset: Dataset, skip_layers=0, max_samples=None, classifier_opts=None,
             method='montecarlo', method_opts=None, loader_opts=None, bernoulli=False, create_final_embedding: bool = False):
    task2vec_embedder = Task2Vec(probe_network, skip_layers=skip_layers, max_samples=max_samples, classifier_opts=classifier_opts,
                                 method=method, method_opts=method_opts, loader_opts=loader_opts, bernoulli=bernoulli)
    embed = task2vec_embedder.embed(
        dataset, create_final_embedding=create_final_embedding)
    return embed
...
class Task2Vec:
    ...
    def embed(self, dataset: Dataset, create_final_embedding: bool = False):
        ...
        if create_final_embedding:
            return embedding.hessian/embedding.scale
        return embedding
            ...
            for i, (data, target) in enumerate(tqdm(data_loader, leave=False, desc="Computing Fisher")):
                ...
                if self.bernoulli:
                    ...
                    target = torch.multinomial(
                        F.softmax(output, dim=-1), 1).detach().view(-1)
    ...
    def _run_epoch(self, data_loader: DataLoader, model: ProbeNetwork, loss_fn,
        ...
        for i, (input, target) in enumerate(tqdm(data_loader, leave=False, desc="Computing Fisher")):
            ...
            lz = beta * variational.get_compression_loss(
                model) if add_compression_loss else torch.zeros_like(loss)
            loss += lz

            error = get_error(output, target)

            metrics.update(n=input.size(0), loss=loss.item(),
                           lz=lz.item(), error=error)
            if train:
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
    ...
    def _cache_features(self, dataset: Dataset, indexes=(-1,), max_samples=None, loader_opts: dict = None):
        ...
        for hook in hooks:
            hook.remove()
        for index in indexes:
            self.model.layers[index].input_features = torch.cat(
                self.model.layers[index].input_features)
    ...
    def _fit_classifier(self, optimizer='adam', learning_rate=0.0004, weight_decay=0.0001,
        ...
        if not hasattr(self.model.classifier, 'input_features'):
            raise ValueError(
                "You need to run `cache_features` on model before running `fit_classifier`")
        ...
        if optimizer == 'adam':
            optimizer = torch.optim.Adam(
                self.model.fc.parameters(), lr=learning_rate, weight_decay=weight_decay)
        elif optimizer == 'sgd':
            optimizer = torch.optim.SGD(self.model.fc.parameters(
        ...
        for epoch in tqdm(range(epochs), desc="Fitting classifier", leave=False):
            ...
            logging.info(
                f"[epoch {epoch}]: " + "\t".join(f"{k}: {v}" for k, v in metrics.avg.items()))
...
def _get_loader(trainset, testset=None, batch_size=64, num_workers=6, num_samples=10000, drop_last=True):
    ...
    num_workers = num_workers if not isinstance(
        trainset, torch.utils.data.TensorDataset) else 0
