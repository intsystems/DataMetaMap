...
def get_compression_loss(model):
    ...
    w_norm2 = sum([x.weight.pow(2).sum() / x.loglambda2.exp()
                  for x in modules])
...
def variational_forward(module, input):
    ...
    if isinstance(module, torch.nn.modules.conv.Conv2d):
        ...
        raise NotImplementedError(
            "Module {} not implemented.".format(type(module)))
...
def _reset_logvar(module, variance_scaling=0.05):
    if hasattr(module, 'logvar0'):
        ...
        var = w.view(w.size(0), -1).var(dim=1).view(-1, *
                                                    ([1] * (w.ndimension() - 1)))  # .expand_as(w)
