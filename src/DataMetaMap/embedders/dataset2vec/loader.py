...
class Dataset2VecLoader(BaseDatasetLoader):
    """Loader для Dataset2Vec.

    На каждой итерации возвращает:
        (X1, y1, X2, y2, label)
    где label=1 если оба сэмпла из одного датасета.
    """
    ...
    def __init__(
        ...
        super().__init__(batch_size, n_batches)
        self.Xs: list[Tensor] = []
        self.ys: list[Tensor] = []
    ...
    def _get_random_dataset_indices(self) -> tuple[int, int]:
        """С вероятностью 0.5 — один датасет (positive pair)."""
    ...
    def _sample_indices(
        ...
        """
        Сэмплирует индексы строк (степень двойки от 8 до 256),
        случайное подмножество признаков и целей.
        """
        n_rows = X.shape[0]
...
class RepeatableDataset2VecLoader(Dataset2VecLoader):
    """
    Версия Dataset2VecLoader которая возвращает
    одинаковые батчи при каждом вызове iter().
    Используется для валидации и тестирования.
    """

    def load(self, data):
        super().load(data)
        # Создаем ссылку на метод _get_batch
        get_batch = super()._get_batch
        # Используем её в list comprehension
        self._fixed_batches = [
            get_batch()
