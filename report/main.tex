...
\paragraph{MMD pretrain-selection benchmark.}
The notebook
\path{benchmarks/pretrain_benchmark/mmd/benchmark_mmd.ipynb}
runs the same transfer-selection experiment with the
\texttt{MMDEmbedder}. For every target dataset it computes the pairwise
$\widehat{\mathrm{MMD}}^{2}$ matrix (with a frozen ResNet-18 encoder by
default, falling back to raw pixels as an ablation), recommends the
nearest source dataset, and reports the resulting downstream test
accuracy alongside the random and ImageNet-pretraining baselines from
the Wasserstein and Task2Vec benchmarks.

\paragraph{Status of MMD in the repository.}
MMD is now packaged as a first-class module in
\path{src/data_meta_map/mmd/}, with a top-level
\path{mmd_embedder.py} class implementing the same
\texttt{BaseEmbedder} interface as the other methods. The module ships
the biased and unbiased empirical estimators of
$\widehat{\mathrm{MMD}}^{2}$, a Random Fourier Features mean
embedding for explicit per-dataset vectors, three pluggable
representation backends (raw features, a frozen pretrained encoder,
and a per-dataset autoencoder), unit tests, a demo notebook, and the
benchmark notebook described above.
...
\paragraph{MMD demo.}
The notebook \path{demo/mmd/simple_example.ipynb} validates the
empirical $\widehat{\mathrm{MMD}}^{2}$ estimators on synthetic
Gaussians, sweeps the RBF bandwidth around the median heuristic, and
visualizes pairwise dataset similarity across MNIST, CIFAR-10, KMNIST,
and EMNIST-letters using all three representation backends (raw
pixels, a frozen ResNet-18 encoder, and a per-dataset autoencoder),
making the role of the auxiliary model explicit.
...
repository combines packaged Dataset2Vec, Wasserstein, and MMD
embedders, a Task2Vec subpackage, and benchmark notebooks for
transfer and meta-learning scenarios. The MMD module additionally
ships three pluggable representation backends (raw features, a
frozen pretrained encoder, and a per-dataset autoencoder) that
realise the multi-representation perspective on MMD discussed in the
Methodology section.
...
across all methods and on extending the released result tables with
fully reproducible numeric comparisons.
...
\begin{itemize}
    ...
    \item \textbf{MMD Demo}: A notebook that validates the empirical
    $\widehat{\mathrm{MMD}}^{2}$ estimators on synthetic Gaussians,
    sweeps the RBF bandwidth around the median heuristic, and
    visualizes pairwise dataset similarity for raw pixels, a frozen
    ResNet-18 encoder, and a small per-dataset autoencoder side by
    side.
    ...
    notebooks evaluate Task2Vec, Wasserstein, and MMD retrieval as
    heuristics for choosing a source pretraining dataset before
